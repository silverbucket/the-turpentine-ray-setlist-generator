...
SetList.prototype._buildSongBiases = function (songs) {
    const magnitude = clampFloat(this._randomness.songBias, DEFAULT_RANDOMNESS.songBias, 0);
...
SetList.prototype._songBias = function (songId) {
...
SetList.prototype._selectBeamStates = function (nextStates) {
    if (nextStates.length <= this._options.beamWidth) {
        return nextStates.slice();
    }

    const multiplier = clampInteger(this._randomness.beamChoicePoolMultiplier, DEFAULT_RANDOMNESS.beamChoicePoolMultiplier, 1);
    ...
    const temperature = clampFloat(this._randomness.beamTemperature, DEFAULT_RANDOMNESS.beamTemperature, 0.01);
    const maxStatesPerLastSong = clampInteger(
        this._randomness.maxStatesPerLastSong,
        DEFAULT_RANDOMNESS.maxStatesPerLastSong,
        1
    );
...
SetList.prototype._diversifyCompatibleRuns = function (items) {
...
SetList.prototype._reorderRun = function (runItems, startPosition) {
    const remaining = runItems.slice();
    const ordered = [];
    const temperature = clampFloat(
        this._randomness.blockShuffleTemperature,
        DEFAULT_RANDOMNESS.blockShuffleTemperature,
        0.01
    );
    ...
    for (let offset = 0; offset < runItems.length; offset += 1) {
        ...
        const weights = ranked.map((entry) => {
            return Math.exp(-(entry.score - bestScore) / temperature);
        });
        const total = weights.reduce((sum, weight) => sum + weight, 0);
        let target = this._rng() * total;
...
SetList.prototype._performanceSignature = function (performance) {
...
SetList.prototype._finalizeItems = function (items) {
